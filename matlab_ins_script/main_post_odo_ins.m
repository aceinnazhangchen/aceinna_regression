...
clear;
clc;
filesname = 'E:/WorkSpace/aceinna_regression_workspace/result_data/git-version/openrtk_log_20210308_165345/';
...
if(1 == find_ref && 1 == find_config)
   for i = 1 : (filesize - 2)
      if(strfind(filesnamelist(i+2).name,'dev'))
        ...
        need_to_be = 0;
        for k = 1 : (filesize2 - 2)
             if(strfind(devnamefolderlist(k+2).name,'_result_odo.txt'))
        ...
        if (1 <= need_to_be)
            ...
            k = strfind(filesname,'/');
            cfg.outputfilefolder = [filesname(1:k(end-1)) filesname(k(end-1):k(end)) 'o_odo_post_turn_result/' filesnamelist(i+2).name  '/'];
